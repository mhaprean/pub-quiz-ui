...
interface IPropsCupIcon {
  place: number;
}

const StyledCupIcon = styled('div')`
  display: flex;
  align-items: center;
  svg {
    width: 40px;
...
const CupIcon = ({ place }: IPropsCupIcon) => {
  const firstPlace = (
  ...
  return (
    <StyledCupIcon className="CupIcon">
      {place === 1 && firstPlace}
      {place === 2 && secondPlace}
      {place === 3 && thirdPlace}
      {place > 3 && (
        <Typography variant="subtitle1" className="place">
          {place}.
        </Typography>
      )}
    </StyledCupIcon>
